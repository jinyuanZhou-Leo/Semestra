...
| PLUGIN_DEVELOPMENT.md | Plugin guide | Current plugin authoring guide covering `plugin.ts`, `index.ts`, optional `settings.tsx`, optional `setup.tsx`, generated backend manifests, `settings_key`-backed setup/settings persistence, the public `@/plugin-sdk` surface, and private `host-policy.json`. |
