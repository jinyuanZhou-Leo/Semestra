...
| PLUGIN_DEVELOPMENT.md | Plugin guide | Current descriptor-first plugin authoring guide covering `plugin.json`, `plugin.ts`, `index.ts`, optional `settings.tsx`, optional `setup.schema.json`, the public `@/plugin-sdk` surface, private `host-policy.json`, and the split between host-managed config, runtime settings, and Semester setup schema. |
| plugin-inheritance-architecture.md | Architecture note | Mermaid-backed explanation of the separate activation and settings chains, including Program installation, Semester activation, unassigned-Course activation, Course-scoped settings, runtime assembly, and host-managed plugin config resolution. |
