...
| PLUGIN_DEVELOPMENT.md | Plugin guide | Developer guide for backend-owned plugin identity/governance, frontend plugin manifests and contribution catalogs, plugin setup DSL authoring plus manifest generation, runtime/global-settings registration, wizard setup contracts, and runtime host APIs such as confirmed tab jumps and plugin-local UI-state caching. |
