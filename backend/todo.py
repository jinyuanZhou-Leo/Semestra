# input:  [SQLAlchemy session, backend ORM/schema/color helper modules, and legacy builtin-event-core todo tab settings]
# output: [semester-scoped todo domain helpers for migration, CRUD mutations, validation, and API payload assembly]
# pos:    [Backend domain service that owns persisted Todo tables, stores task data without backend ordering, bridges legacy tab.settings storage into the semester-level model, and resolves stable Program-derived course colors for Todo payloads]
#
...
def _resolve_course_display_color(
    ...
    *,
    resolved_subject_color_map: dict[str, str],
) -> str:
    if is_hex_color(course.color):
        return str(course.color)

    subject_code = resolve_subject_code(
        category=course.category,
        alias=course.alias,
        name=course.name,
    )
    return resolved_subject_color_map.get(subject_code, "") if subject_code else ""


def _build_resolved_subject_color_map(courses: list[models.Course], raw_subject_color_map: str | None) -> dict[str, str]:
    discovered_subject_codes = [
        resolve_subject_code(category=course.category, alias=course.alias, name=course.name)
        for course in courses
    ]
    return resolve_subject_color_assignments(
        discovered_subject_codes,
        parse_subject_color_map(raw_subject_color_map),
    )
...
def _build_course_snapshots(semester: models.Semester) -> list[CourseSnapshot]:
    resolved_subject_color_map = _build_resolved_subject_color_map(
        list(semester.courses),
        semester.program.subject_color_map if semester.program else None,
    )
    return [
        CourseSnapshot(
            course_id=course.id,
            ...
            course_color=_resolve_course_display_color(course, resolved_subject_color_map=resolved_subject_color_map),
...
def _serialize_state(semester: models.Semester) -> schemas.TodoSemesterState:
    ...
    tasks = [
        schemas.TodoTask(
            ...
            course_color=(
                _resolve_course_display_color(course_map[task.course_id], resolved_subject_color_map=resolved_subject_color_map)
    ...
    course_options = [
        schemas.TodoCourseOption(
            ...
            color=_resolve_course_display_color(course, resolved_subject_color_map=resolved_subject_color_map),
        )
        for course in sorted(semester.courses, key=lambda item: (item.name or "").lower())
    ]
