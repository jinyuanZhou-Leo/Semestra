# input:  [unittest, in-memory SQLAlchemy setup, backend CRUD helpers, schemas, and plugin management contract]
# output: [backend regression tests covering Program/Semester plugin management, Program-enabled plugin visibility across Semester settings, Program-disabled plugin hiding, manifest-backed plugin-system setup flows, contribution-context filtering, guarded runtime-tab settings writes, migration backfills for V2 tab settings, legacy homepage-tab normalization, resolved-config rules, and draft review lifecycle enforcement plus the single-draft database invariant]
# pos:    [backend unit tests for Program/Semester plugin management, Program-enabled or Program-disabled Semester visibility rules, plugin-system setup contracts, surface-aware runtime filtering, guarded V2 tab-setting persistence, migration backfills, legacy Semester tab normalization, and draft flows plus draft-uniqueness enforcement without requiring a running Semestra server]
#
...
class PluginGovernanceDraftTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.user = models.User(email="plugin-management@example.com", hashed_password="hashed", user_setting="{}")
    ...
    def test_semester_runtime_filters_contributions_by_context_and_ignores_stale_selected_tabs(self) -> None:
        ...
        self.assertNotIn("course-list", widget_types)
    ...
    def test_bulk_enabling_canvas_plugin_succeeds_without_lms_prerequisite(self) -> None:
        program = self._create_program()
        crud.upsert_program_plugin_installation(
            self.db,
            program.id,
        ...
        activation = next(item for item in updated_draft["plugin_activations"] if item["plugin_id"] == "builtin-canvas-integration")
        self.assertTrue(activation["is_enabled"])

    def test_non_host_shell_plugins_are_not_locked_by_default(self) -> None:
        program = self._create_program()
        semester = crud.create_semester(
            self.db,
            schemas.SemesterCreate(
        ...
        self.assertEqual([item for item in activations if item["plugin_id"] == "course-list"], [])
        self.assertTrue(all(not item["locked"] for item in activations))
    ...
    def test_assigned_course_rejects_course_level_plugin_registry(self) -> None:
        program = self._create_program()
        semester = crud.create_semester(
            self.db,
            schemas.SemesterCreate(
