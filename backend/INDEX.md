...
The backend now exposes generic LMS course import, Course link/relink/unlink, and read-only Assignment plus Calendar APIs without leaking secrets, with Canvas implemented through a REST adapter.
...
| lms_canvas.py | Canvas adapter | Implements the Canvas provider adapter on top of Canvas REST endpoints for validation, course discovery, assignment reads, and semester-scoped calendar-event reads. |
