...
| main.py | API entry point | Defines FastAPI app, middleware, HTTP route handlers, auth login/logout cookie issuance, and semester Reading Week validation/schema compatibility shims. |
...
| models.py | ORM models | Defines SQLAlchemy table models and relational constraints, including optional semester Reading Week dates. |
...
| schemas.py | API schema layer | Defines request/response validation models, including semester Reading Week fields and strict widget `layout_config` shape/range validation. |
