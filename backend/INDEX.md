...
The backend now exposes generic LMS course import, Course link/relink/unlink, ICS-backed semester/course imports, and read-only Assignment plus Calendar APIs without leaking secrets, with Canvas implemented through a REST adapter that now owns provider-specific config normalization and credential masking before data reaches shared LMS consumers.
...
| lms_service.py | LMS orchestration | Owns multi-integration persistence, Program integration selection checks, Course link lifecycle, provider-backed integration payload normalization, LMS course import flows, provider dispatch, and read-only Assignment/Calendar aggregation. |
...
| test_lms_integrations.py | Unit test script | Verifies multi-integration LMS storage, Program/Course link constraints, provider-backed import flows, program-level course stat/reassignment safeguards, and read-only Assignment/Calendar responses. |
