...
The backend now exposes generic LMS course import, Course link/relink/unlink, ICS-backed semester/course imports, and read-only Assignment plus Calendar APIs without leaking secrets, with Canvas implemented through a REST adapter that normalizes provider-specific due-date recognition before data reaches shared LMS consumers.
Course-scoped widget/tab ownership checks now resolve through `Course -> Program` directly so Program-level courses without a bound semester still support Course homepage initialization and tab/widget CRUD.
...
| lms_canvas.py | Canvas adapter | Implements the Canvas provider adapter on top of Canvas REST endpoints for validation, course discovery, assignment reads, provider-specific due-date normalization, and semester-scoped calendar-event reads. |
...
| test_lms_integrations.py | Unit test script | Verifies multi-integration LMS storage, Program/Course link constraints, import flows, and read-only Assignment/Calendar responses. |
