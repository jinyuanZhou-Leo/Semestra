...
| main.py | API entry point | Defines FastAPI app, middleware, HTTP route handlers, auth login/logout cookie issuance, multi-integration LMS management routes, validated Program/Course LMS import-link-sync and course reassignment endpoints, ICS-backed semester/course upload imports, Program subject-color-map persistence, account-wide course-resource upload/list/link/create/rename/delete/download APIs, backup import/export, semester Reading Week validation, semester todo APIs, plugin shared settings endpoints, gradebook APIs, range-based semester calendar schedule reads, and force-aware widget/tab ownership checks that also work for Program-level courses without runtime schema rewrite helpers. |
