...
`widget.tsx` owns the per-instance streak store, delete cleanup hook, concise mode-specific settings, and check-in orchestration.
...
| widget.tsx | Widget runtime | Implements streak window helpers, per-instance streak store, delete cleanup via `onDelete`, compact untitled fallback text, concise mode-specific settings panels, ring-only motivational toast gating, single-shell widget chrome, and reset behavior via headerButtons. |
| widget.test.tsx | Test suite | Covers helper math, per-instance normalization, dual widget rendering, isolated multi-instance behavior, mode-specific settings surfaces, single-shell chrome, and header reset/delete behavior. |
