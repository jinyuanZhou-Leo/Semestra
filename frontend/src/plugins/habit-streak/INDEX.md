...
`plugin.ts` now provides the eager typed descriptor-backed entry while `index.ts` lazily exposes both runtime widget definitions.
...
| plugin.ts | Plugin entry | Single typed plugin authoring entry that declares dual-widget metadata and lazy runtime loading through the frontend plugin SDK. |
