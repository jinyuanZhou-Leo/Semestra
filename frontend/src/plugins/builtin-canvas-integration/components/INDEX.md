...
| CanvasModulesView.tsx | Module view | Renders Canvas module sections as expanded-by-default collapsible cards that window offscreen sections, consume inline item summaries from the modules payload instead of issuing one list request per module, keep supported module items in-app with native detail drill-down plus configured-client native blob-backed file rendering, reject app-shell HTML downloads, restore list/detail scroll position, and only open External or Discussion items in Canvas. |
