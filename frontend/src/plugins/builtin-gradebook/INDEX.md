...
Runtime UI consumes a slim fact-only API while shared helpers derive summaries, forecasts, and plan recommendations on the client, now blocking all Gradebook math until total assessment weight reaches 100%, including shared TanStack Query caching reused by both the tab and settings surface, a CSS-only responsive course-only metrics widget aligned with workspace stat styling and stacked vertically to fill widget height, plugin-local one-decimal GPA-percentage display formatting via `shared.ts`, and standardized semantic empty states for create, no-results, and unavailable flows.
...
| shared.ts | Shared helpers | Exposes plugin constants, client-side forecast/plan calculators, total-weight gating that disables calculations below 100%, plugin-local GPA-percentage/date/percent formatters, slate-consistent fallback color helpers, and contrast-safe hex badge styling. |
