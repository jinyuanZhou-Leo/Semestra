...
| settings.tsx | Plugin settings | Provides course-level forecast mode radio selection with clearer guidance plus an Event-types-style category management table and a shadcn `Field`-based category form dialog for the Gradebook plugin, now sharing the same query-backed gradebook cache as the tab. |
