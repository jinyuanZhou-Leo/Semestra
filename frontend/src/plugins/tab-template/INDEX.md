...
`plugin.ts` publishes the typed catalog entry, `settings.tsx` owns the tab settings UI, `setup.tsx` demonstrates a DSL-only Semester setup that covers every host-rendered field type plus validation hooks and now acts as the single setup source of truth for both frontend wizard rendering and backend schema generation, and `tab.tsx` stays focused on runtime rendering.
