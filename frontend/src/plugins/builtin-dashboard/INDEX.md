...
`plugin.json` and `plugin.ts` now act as the descriptor-backed entry layer while runtime code keeps split layout sync/commit callbacks.

| File | Role | Description |
|------|------|-------------|
| INDEX.md | Architecture index | Local map for built-in dashboard plugin files and responsibilities. |
| plugin.json | Plugin manifest | Static public manifest for plugin identity plus dashboard-tab metadata. |
