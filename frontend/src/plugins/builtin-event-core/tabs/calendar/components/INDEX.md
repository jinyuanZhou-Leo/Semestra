...
| CalendarAllDayEventContent.tsx | Event UI | All-day event renderer that handles tighter month pills, week all-day rows, and todo-style completion radios with icon sizing aligned to rounded pill chrome. |
...
| CalendarStandardEventContent.tsx | Event UI | Standard timed-event renderer that handles month chips and week-detail cards with unified location/conflict/time metadata rows. |
