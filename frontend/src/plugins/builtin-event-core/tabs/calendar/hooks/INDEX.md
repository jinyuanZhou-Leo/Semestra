...
These hooks split semester context, navigation, source loading, and editing into low-coupling units with Query-cache-aware reload behavior, cache-first event preservation, and persisted navigation UI state.
...
| useCalendarNavigationState.ts | Navigation hook | Owns persisted week/month state, DST-safe academic week labels, buffered query windows, adjacent-window prefetch ranges, and user-navigation rules independent from source loading and week-view layout. |
| useCalendarSources.ts | Source orchestration hook | Hydrates registered Calendar sources from cache first, stabilizes repeated source arrays by source identity, revalidates sources when they are re-enabled, lets sources invalidate their own caches, progressively commits each source as it resolves, and supports targeted refreshes without full cold reloads or remount skeleton flashes. |
