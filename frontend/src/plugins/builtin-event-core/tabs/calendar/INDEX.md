...
| EventEditor.tsx | Event editor | Per-event detail dialog with source badges, wrapperless event metadata, occurrence-scoped conflict details, LMS/read-only detail mode, safe text/list fallback for LMS descriptions unless rich HTML is explicitly enabled, a scroll-safe long-content body with fixed actions, and skip-save wiring for editable schedule events. |
