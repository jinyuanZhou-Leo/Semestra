...
| CalendarSettingsSection.tsx | Active settings section | Settings-page Calendar controls including shadcn `Field`-based standard form layout, an integrated source list with per-source enable toggles/colors/source settings, LMS description risk confirmation, right-aligned general toggle rows, draft-friendly time-window inputs, week-view screen width, Reading Week numbering, export, and reset actions. |
