...
| FullCalendarView.tsx | Calendar renderer | FullCalendar adapter that maps event-core state into week/month views with configurable week-view horizontal scroll width, named all-day and standard event-content subcomponents that each handle their own month/week variants, all-day todo completion radios that sync back to Todo, compact trailing month time labels, `x more` month overflow labels, a screenshot-style labeled current-time indicator, matched red today badges across month and week headers, recurring icons, conflict-aware lane splitting, a single-shell border treatment, refined scrollbars, softer tinted today backgrounds, softer tinted event blocks with retained source colors, capped scrollable all-day rows with hidden scrollbars, all-day todo rows, full-width single-event lanes, overflow-to-week transitions, and remount-based view/date sync backed by DST-safe academic week math. |
| SemesterScheduleExportModal.tsx | Export workflow | Schedule export modal with PNG/PDF/ICS generation helpers and filters. |
| components/ | Local subcomponents | Calendar-specific presentational helpers such as shared event-content rendering, skeletons, and source-color inputs. |
