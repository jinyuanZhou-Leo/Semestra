...
It now orchestrates a semester-first aggregate todo view backed by the dedicated semester todo API and shared semester todo query cache instead of `tab.settings`, with local-only sorting persisted per list through plugin UI state, note-based task details, inline editing, empty-title direct task deletion, and keyboard-safe destructive flows.
...
| TodoTab.tsx | Runtime entry | Main container that hydrates from shared semester todo query state, restores local per-list sorting preferences through plugin UI state, drives task/section mutations through dedicated endpoints, rebuilds local list state from canonical cache after Calendar-originated todo updates, lets each inline composer own its local draft while the tab only handles persistence, bypasses delete confirmation when inline title edits are cleared, and uses browser-safe completion timer handle typing for strict TS builds. |
| TodoTab.test.tsx | Interaction tests | Covers per-composer inline draft isolation, empty-title inline task deletion, canonical-to-UI todo derivation, stable compact inline create shell reuse, blur-triggered quick-create commits, compact time-chip editing alignment, local sort bucketing, and persisted view-preference restoration. |
