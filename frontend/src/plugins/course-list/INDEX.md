...
| globalSettings.tsx | Plugin settings UI | Provides shared course-management controls with semester load retry UI, stale-response guards, a mobile-safe `DataTable`-aligned semester course table, consistent one-decimal GPA-percentage display via a plugin-local formatter, row-actions dropdown-based course removal, and a disabled manager entry until semester/program data is valid while remaining compatible with framework-managed plugin settings props. |
