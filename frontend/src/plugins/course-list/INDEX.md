...
| globalSettings.tsx | Plugin settings UI | Provides shared course-management controls with semester load retry UI, stale-response guards, a `CrudPanel`-aligned semester course table, and a disabled manager entry until semester/program data is valid while remaining compatible with framework-managed plugin settings props. |
