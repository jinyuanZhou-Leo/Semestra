...
|------|------|-------------|
| CourseHomepage.tsx | Route page | Course workspace route that wires dashboard widgets with split layout local-sync/commit persistence callbacks, non-wrapping/truncating breadcrumb placeholders, focus-safe add-widget/add-tab overlay triggers, and skeleton-first lazy tab loading with loaded-state fade-in. |
...
| SemesterHomepage.tsx | Route page | Semester workspace route that wires dashboard widgets with split layout local-sync/commit persistence callbacks, non-wrapping/truncating breadcrumb placeholders, focus-safe add-widget/add-tab overlay triggers, and skeleton-first lazy tab loading with loaded-state fade-in. |
