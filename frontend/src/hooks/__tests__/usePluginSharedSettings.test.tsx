...
// pos:    [hook-level regression tests for Program, Semester, and Course plugin shared settings persistence]
//
...
const { apiMock } = vi.hoisted(() => ({
  apiMock: {
    getProgramPluginInstallations: vi.fn(),
...
describe('usePluginSharedSettings', () => {
  beforeEach(() => {
    apiMock.getProgramPluginInstallations.mockReset();
  ...
  it('loads Program plugin settings from Program installations and syncs updates through Program governance', async () => {
    apiMock.getProgramPluginInstallations.mockResolvedValue([
      {
        id: 'installation-1',
        plugin_id: 'course-list',
        display_name: 'Course List',
        description: 'Program course list settings.',
        long_description: '',
        author: 'Semestra',
        ...
        capabilities: {},
        setup_sections: [],
        program_settings: { allowCourseCreation: true },
        resolved_program_settings: { allowCourseCreation: true },
        fields: [],
        available: true,
        installed: true,
      },
    ]);
    apiMock.upsertProgramPluginInstallation.mockImplementation(
      async (_programId: string, pluginId: string, data: { program_settings?: Record<string, unknown> }) => ({
        id: 'installation-1',
        plugin_id: pluginId,
        display_name: 'Course List',
        description: 'Program course list settings.',
        ...
        capabilities: {},
        setup_sections: [],
        program_settings: data.program_settings ?? {},
        resolved_program_settings: data.program_settings ?? {},
        fields: [],
        available: true,
        installed: true,
      })
    ...
    expect(result.current.settings).toEqual({ allowCourseCreation: true });

    act(() => {
      result.current.updateSettings({ allowCourseCreation: false });
    });

    await waitFor(() => {
      expect(apiMock.upsertProgramPluginInstallation).toHaveBeenCalledWith(
        'program-1',
        'course-list',
        { program_settings: { allowCourseCreation: false } }
      );
    });
  });
