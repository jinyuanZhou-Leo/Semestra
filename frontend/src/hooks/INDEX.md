...
| useDashboardWidgets.ts | Hook module | Orchestrates widget CRUD with add-success signaling, unavailable-widget force cleanup, resilient settings/layout parsing, immediate optimistic-update rollback on save failure, context-key reset safeguards, and split local layout sync with parallelized commit persistence. |
