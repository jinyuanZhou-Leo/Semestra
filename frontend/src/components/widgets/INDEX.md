...
Includes deterministic fallback placement, lower-frequency resize width commits, zero-width mount guards, interaction-scoped local-sync plus commit-persistence layout flows, plugin loading skeleton/fade handling, and forced delete affordances for unavailable widgets.
...
| DashboardGrid.tsx | UI component | Uses RGL v2 `useContainerWidth` (`1w = 1h`) with lower-frequency stabilized width commits, zero-width render guards on tab remount, interaction-scoped local layout sync, drag/resize commit persistence callbacks, and a dedicated unavailable-widget delete channel. |
