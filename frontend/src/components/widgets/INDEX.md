...
| DashboardWidgetWrapper.tsx | UI component | Builds plugin header actions with shared moderate-opacity glassmorphism button styling, wraps widget content in runtime instance scope for plugin-local UI state, keeps known lazy plugin widgets on bordered skeletons until runtime registration completes, and preserves delete affordances when widget plugins become unavailable. |
