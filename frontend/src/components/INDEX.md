...
| ProgramPluginGovernancePanel.tsx | UI component | Program-level plugin governance settings card that wraps a shared data table with backend-sourced plugin identity plus frontend manifest icons, menu-based row actions, a responsive full-catalog Marketplace surface that keeps installed plugins visible but non-installable, and broad Semester/Course cache invalidation after Program plugin changes. |
...
| SemesterPluginGovernancePanel.tsx | UI component | Semester-level plugin governance settings card that lists every Program-enabled plugin, shows frontend manifest icons, lets not-yet-imported Semester plugins appear as Off rows, and intentionally exposes only enable/disable controls so Semester plugin data is never deleted from Settings. |
