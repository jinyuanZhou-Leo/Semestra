...
// output: [`CoursePluginGovernancePanel` component]
...
interface CoursePluginGovernancePanelProps {
  courseId: string;
  pluginActivations: CoursePluginActivation[];
  onChanged?: () => Promise<void> | void;
}

const isLocalDisableReason = (plugin: CoursePluginActivation) => plugin.availability_reason === "Disabled for this Course.";

export const CoursePluginGovernancePanel: React.FC<CoursePluginGovernancePanelProps> = ({
  courseId,
  pluginActivations,
  onChanged,
}) => {
  const queryClient = useQueryClient();
  const [detailPlugin, setDetailPlugin] = useState<CoursePluginActivation | null>(null);
  const [togglingPluginId, setTogglingPluginId] = useState<string | null>(null);

  const installedPlugins = useMemo(
    () => pluginActivations.filter((plugin) => plugin.source === "course"),
    [pluginActivations],
  );
  ...
  const invalidateAll = async () => {
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.courses.pluginSettings(courseId) }),
    ]);
    await onChanged?.();
  };

  const handleToggleEnabled = async (item: CoursePluginActivation, nextEnabled: boolean) => {
    setTogglingPluginId(item.plugin_id);
    try {
      await api.upsertCoursePluginActivation(courseId, item.plugin_id, {
  ...
  const handleToggleAllEnabled = async (nextEnabled: boolean) => {
    ...
    try {
      await api.bulkUpdateCoursePluginActivations(courseId, {
  ...
  return (
    <SettingsSection
      ...
      description="Enable Program-managed plugins for this Course while it is not assigned to any Semester."
      contentClassName="space-y-5"
    >
      <DataTable
        title="Course Plugins"
        description="Only plugins that support unassigned Courses appear here. Enabling a plugin affects this Course only."
        items={installedPlugins}
        emptyMessage="No Program plugins are available for this unassigned Course."
            ...
            <TableHead className="w-[180px] text-right">
              <div className="ml-auto flex w-full max-w-[172px] items-center justify-end gap-3">
                ...
                <Switch
                  ...
                  aria-label="Toggle all editable Course plugins"
