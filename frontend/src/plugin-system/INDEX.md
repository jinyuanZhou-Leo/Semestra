...
Bridges backend-owned plugin identity with frontend-owned manifests and setup DSL so governance UIs read `name/description/author` from APIs while the runtime still resolves plugin icons, contribution catalogs, generated setup contracts when present, and tab-to-plugin ownership locally.
...
| index.test.tsx | Test file | Covers plugin-manifest icon exposure, runtime instance settings resolution, absence of Eventcore setup registration, eager plugin-global settings exposure, and idle-time background preloading through the public facade. |
