...
Bridges backend-owned plugin identity with frontend-owned manifests and setup DSL so governance UIs read `name/description/author` from APIs while the runtime still resolves plugin icons, contribution catalogs, generated setup contracts, and tab-to-plugin ownership locally.
