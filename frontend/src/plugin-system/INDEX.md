...
Plugin manager facade for eager manifest-backed plugin discovery, lazy runtime registration, runtime load-state tracking, workspace-local plugin-host navigation, plugin runtime instance scope, and plugin-local UI-state caching.
It consumes `plugins/*/plugin.ts` plus public `plugin.json`, private `host-policy.json`, and optional `setup.schema.json`, keeping the backend and frontend aligned on one static contract source instead of generated manifests.
...
| setupRegistry.ts | Setup registry | Eagerly scans `plugins/*/plugin.ts`, validates each setup declaration against its descriptor-backed plugin definition, and keeps custom UI/validation hooks available to the frontend wizard. |
